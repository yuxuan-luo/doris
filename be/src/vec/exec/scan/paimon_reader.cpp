...
PaimonJniReader::PaimonJniReader(const std::vector<SlotDescriptor*>& file_slot_descs,
    ...
    params["size_byte"] = range.table_format_params.paimon_params.paimon_split.size();
    LOG(INFO) << "FORMAT_JNI paimon_split:" << params["split_byte"];
    LOG(INFO) << "FORMAT_JNI paimon_split:" << params["length_byte"];
    LOG(INFO) << "FORMAT_JNI paimon_split:" << params["size_byte"];
